...
function formatServiceName(raw: string): string {
  ...
  if (lower.includes("wifi") || lower.includes("wi-fi") || lower.includes("network")) return "Wi-Fi & Networking";
  if (lower.includes("intercom")) return "Video Intercom & VDP";
  if (lower.includes("biometric") || lower.includes("access control")) return "Biometric Access Control";
          ...
          <div className="admin-card">
            ...
            {recentLeads.length === 0 ? (
              <div className="empty-state" style={{ padding: "36px 20px" }}>
                <div className="empty-state-icon-wrap" style={{ width: "48px", height: "48px", marginBottom: "12px" }}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                    <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
                    <circle cx="9" cy="7" r="4" />
                  </svg>
                </div>
                <h4 style={{ margin: "0 0 6px", fontSize: "16px", color: "var(--primary)" }}>No leads recorded yet</h4>
                <p style={{ margin: "0 0 16px", fontSize: "13px" }}>Customer enquiries from the website or direct calls will appear here.</p>
                <Link href="/admin/leads?new=1" className="admin-btn admin-btn-small admin-btn-primary">
                  + Add First Lead
                </Link>
              </div>
            ) : (
                      ...
                      {recentLeads.map((lead) => (
                        <tr key={lead.id}>
                          <td>
                            ...
                            <div className="table-subtext" style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                          ...
                          <td>
                            <span style={{ fontWeight: 500 }}>{formatServiceName(lead.serviceRequested)}</span>
                          </td>
                          <td>
                  ...
                  {recentLeads.map((lead) => (
                    <div key={lead.id} className="mobile-project-card" style={{ padding: "14px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <div>
                          <strong style={{ fontSize: "15px", color: "var(--primary)" }}>{lead.contactName}</strong>
                          <div style={{ fontSize: "12px", color: "var(--muted)", marginTop: "2px" }}>{lead.phone}</div>
                        </div>
                        <span className={`status-badge badge-${lead.status.toLowerCase()}`}>
                          {lead.status}
                        </span>
                      </div>
                      <div style={{ fontSize: "13px", fontWeight: 500, color: "var(--ink)", marginTop: "6px" }}>
          ...
          <div className="admin-card">
            ...
            {recentProjects.length === 0 ? (
              <div className="empty-state" style={{ padding: "36px 20px" }}>
                ...
                <h4 style={{ margin: "0 0 6px", fontSize: "16px", color: "var(--primary)" }}>No projects yet</h4>
                <p style={{ margin: "0 0 16px", fontSize: "13px" }}>Create a project to track site survey, costing, procurement, installation, and handover.</p>
                <Link href="/admin/projects?new=1" className="admin-btn admin-btn-small admin-btn-primary">
                  + Create Project
                </Link>
              </div>
            ) : (
              <>
                    ...
                    <tbody>
                      {recentProjects.map((project) => (
                        <tr key={project.id}>
                          ...
                          <td>
                            <span className={`status-badge badge-${project.operationalStatus.toLowerCase()}`}>
                              {project.operationalStatus.replace(/_/g, " ")}
                            </span>
                          </td>
                          <td>
                            <span className="milestone-badge" style={{ fontSize: "11px", padding: "3px 8px" }}>
                ...
                <div className="mobile-cards-list" style={{ marginTop: "8px" }}>
                  {recentProjects.map((project) => (
                    <div key={project.id} className="mobile-project-card" style={{ padding: "14px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <div>
                          <strong style={{ fontSize: "15px", color: "var(--primary)" }}>{project.name}</strong>
                          <div style={{ fontSize: "12px", color: "var(--muted)", marginTop: "2px" }}>{project.customerName}</div>
                        </div>
                        <span className={`status-badge badge-${project.operationalStatus.toLowerCase()}`}>
                          {project.operationalStatus.replace(/_/g, " ")}
                        </span>
          ...
          <div className="account-info-group">
            ...
            <div className="account-title-group">
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <h3 id="account-heading" style={{ margin: 0 }}>Administrator Session</h3>
                <span className="account-role-badge">Administrator</span>
              </div>
              <p className="account-email" style={{ margin: "2px 0 0" }}>{actor.email}</p>
            </div>
          </div>
          <SessionControls />
        </section>
